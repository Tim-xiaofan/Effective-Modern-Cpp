...
#include <cassert>
#include <fstream>
#include "threadsafe_lookup_table2.h"
#include "test_lookup_table_utils.h"

namespace
{

void test_threadsafe_lookup_table2(void)
{
    threadsafe_lookup_table2<int> t;
    assert(t.empty());
    assert(!t.find(1));
...
{
	threadsafe_lookup_table2 <std::int64_t> t;
...
int main(int argc, char* argv[])
{
    ...
    if(argc != 3)
	{
		std::cerr << "Usage "<< argv[0] << " <num_keys> <mode>\n";
		exit(-1);
	}
